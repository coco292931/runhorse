...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path(__file__).with_name("recognize_printed_marker_tkinter_config.json"),
        help="GUI tuning config JSON path.",
    )
...
class RealtimeRecognizerApp:
    def __init__(self, root: tk.Tk, args: argparse.Namespace) -> None:
        ...
        self.config_status_message: str | None = None
        ...
        self.control_integer_only: dict[str, bool] = {}

        self.status_var = tk.StringVar(value=self.config_status_message or "Ready")
        self.class_var = tk.StringVar(value="-")
        self.conf_var = tk.StringVar(value="-")
        self.top1_var = tk.StringVar(value="-")
        self.top1_conf_var = tk.StringVar(value="-")
        self.fps_var = tk.StringVar(value="FPS -")
        self.crop_resolution_var = tk.StringVar(value="Crop -")
        ...
        ttk.Label(preview_frame, text="Selected image").grid(row=2, column=0, sticky="w", pady=(12, 0))
        ttk.Label(preview_frame, textvariable=self.crop_resolution_var).grid(row=2, column=1, sticky="e", pady=(12, 0))
        self.crop_label = ttk.Label(preview_frame)
        self.crop_label.grid(row=3, column=0, columnspan=2, sticky="nsew")

        info_frame = ttk.Frame(preview_frame)
        info_frame.grid(row=4, column=0, columnspan=2, sticky="ew", pady=(12, 0))
        ...
        ttk.Button(button_row, text="Save config", command=self.save_config).grid(row=0, column=2, padx=(0, 8))
        ttk.Button(button_row, text="Defaults", command=self.restore_default_config).grid(row=0, column=3, padx=(0, 8))
        ttk.Button(button_row, text="Exit", command=self.on_close).grid(row=0, column=4)
        ...
        postprocess_frame = ttk.LabelFrame(controls_frame, text="Selected image postprocess", padding=10)
        postprocess_frame.grid(row=2, column=0, sticky="nsew", pady=(12, 0))
        postprocess_frame.columnconfigure(1, weight=1)

        postprocess_specs = [
            ("imgsz", "YOLO imgsz", 32.0, 640.0, float(self.args.imgsz), True),
            ("crop_scale", "Scale", 0.5, 2.0, float(self.args.crop_scale), False),
            ("crop_exposure", "Exposure", 0.2, 2.5, float(self.args.crop_exposure), False),
            ("crop_contrast", "Contrast", 0.2, 3.0, float(self.args.crop_contrast), False),
            ("crop_blur", "Blur", 0.0, 20.0, float(self.args.crop_blur), False),
        ]

        for row, (name, label, start, end, value, integer_only) in enumerate(postprocess_specs):
            self.add_slider(postprocess_frame, row, name, label, start, end, value, integer_only)

    def coerce_control_value(self, name: str, value) -> int | float:
        default = CONFIG_CONTROL_DEFAULTS[name]
        try:
            numeric = float(value)
        except (TypeError, ValueError):
            numeric = float(default)
        if name in INTEGER_CONTROL_NAMES:
            if name == "imgsz":
                size = max(YOLO_IMGSZ_MULTIPLE, int(round(numeric)))
                return ((size + YOLO_IMGSZ_MULTIPLE - 1) // YOLO_IMGSZ_MULTIPLE) * YOLO_IMGSZ_MULTIPLE
            return int(round(numeric))
        return float(numeric)

    def apply_startup_config(self) -> None:
        for name, default in CONFIG_CONTROL_DEFAULTS.items():
            if getattr(self.args, name, None) is None:
        ...
        try:
            ...
            controls = data.get("controls", data) if isinstance(data, dict) else {}
            if not isinstance(controls, dict):
                raise ValueError("controls must be a JSON object")
            controls.pop("crop_sharpness", None)
            for name, value in controls.items():
                if name in CONFIG_CONTROL_DEFAULTS:
                    coerced = self.coerce_control_value(name, value)
                    if name == "imgsz":
                        try:
                            raw_int = int(round(float(value)))
                        except (TypeError, ValueError):
                            raw_int = int(CONFIG_CONTROL_DEFAULTS[name])
                        if coerced != raw_int:
                            print(f"YOLO imgsz adjusted: {raw_int} -> {coerced} ({YOLO_IMGSZ_MULTIPLE}x multiple)")
                    setattr(self.args, name, coerced)
            self.config_status_message = f"Config loaded: {self.config_path.name}"
        except Exception as exc:
            self.config_status_message = f"Config load failed: {exc}"

    def current_config(self) -> dict[str, int | float]:
        return {
            name: self.coerce_control_value(name, getattr(self.args, name, default))
            for name, default in CONFIG_CONTROL_DEFAULTS.items()
        }

    def apply_control_values(self, values: dict[str, int | float]) -> None:
        for name, value in values.items():
            if name not in CONFIG_CONTROL_DEFAULTS:
                continue
            integer_only = name in INTEGER_CONTROL_NAMES
            coerced = self.coerce_control_value(name, value)
            setattr(self.args, name, coerced)
            if name in self.control_vars:
                self.control_vars[name].set(coerced)
                self.update_control_label(name, float(coerced), integer_only)
            if name in CAMERA_CONTROL_PROPS:
                self.apply_camera_control(name)
        self.reset_recognition_cache()

    def save_config(self) -> None:
        payload = {
            "version": CONFIG_VERSION,
            "controls": self.current_config(),
        }
        try:
            self.config_path.parent.mkdir(parents=True, exist_ok=True)
            self.config_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
            self.status_var.set(f"Config saved: {self.config_path.name}")
        except Exception as exc:
            self.status_var.set(f"Config save failed: {exc}")

    def restore_default_config(self) -> None:
        self.apply_control_values(CONFIG_CONTROL_DEFAULTS)
        self.status_var.set("Default config restored")
    ...
    def add_slider(
        ...
        self.control_integer_only[name] = integer_only
    ...
    def on_slider_change(self, name: str, raw_value: str, integer_only: bool) -> None:
        ...
        if integer_only:
            ...
            value = self.coerce_control_value(name, value)
            self.control_vars[name].set(value)
            if name == "imgsz" and value != raw_int:
                message = f"YOLO imgsz adjusted: {raw_int} -> {value} ({YOLO_IMGSZ_MULTIPLE}x multiple)"
                print(message)
        ...
        if name in CAMERA_CONTROL_PROPS:
            self.apply_camera_control(name)
        if name not in CAMERA_CONTROL_PROPS:
            self.reset_recognition_cache()
    ...
    def update_frame(self) -> None:
        ...
        self.update_crop_preview(result.get("crop"))
    ...
    def update_crop_preview(self, crop) -> None:
        if crop is None:
            self.crop_resolution_var.set("Crop -")
            return
        height, width = crop.shape[:2]
        self.crop_resolution_var.set(f"YOLO input {width} x {height}")
        self.update_preview(self.crop_label, crop, is_mask=False, preview_width=self.args.crop_preview_width)

    def update_preview(self, label: ttk.Label, frame, is_mask: bool, preview_width: int | None = None) -> None:
        if frame is None:
            return
        if is_mask:
        ...
        target_width = max(1, preview_width if preview_width is not None else self.args.preview_width)
        if image.width > target_width:
            preview_height = round(image.height * target_width / image.width)
            image = image.resize((target_width, preview_height))
        photo = ImageTk.PhotoImage(image=image)
        label.configure(image=photo)
        if is_mask:
            self.mask_image = photo
        elif label is self.crop_label:
            self.crop_image = photo
        else:
